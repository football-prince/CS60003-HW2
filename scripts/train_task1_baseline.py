...
def main() -> None:
    parser = ArgumentParser(description="Task 1: ImageNet pretrained ResNet baseline.")
    ...
    if args.model not in {"resnet18", "resnet34"}:
        raise ValueError("Task 1 supports --model resnet18 or resnet34.")

    set_seed(args.seed)
    device = select_device(args.device)
    default_output = PROJECT_ROOT / "outputs" / f"task1_{args.model}"
    output_dir = resolve_output_dir(args.output_dir or default_output, overwrite=args.overwrite)
